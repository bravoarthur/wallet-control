...
function SellForm() {
    const { formHandle, saveTradeList, formDisplay } =
        useContext(TradeListContext);
    ...
    useEffect(() => {
        if (!formHandle.name || formHandle.name==='fade') {
            return;
        }
    ...
    function newTransaction(verf: number) {
        const newSell = {
            ...
            valuetion:
                stockSellPrice! * stockSellQtd! -
                (stockList[verf].avaragePrice as number) * stockSellQtd!,
            valuetionPercent:
                ((stockSellPrice as number) / stockList[verf].avaragePrice! -
                    1) *
                100,
            valuetionPY: 0
        };

        newSell.valuetionPY = _handleCalcDate(
            stockList[verf].buyDate,
            stockSellDate,
            newSell.valuetionPercent
        );
            ...
            <FormSell hide={display}>
                ...
                >
                    ...
                    <TextField
                        ...
                        required
                        onChange={(event) =>
                            setStockSellDate(event.target.value)
                        }
                    />
                    ...
                    <TextField
                        ...
                        onChange={(event) =>
                            setStockSellQtd(Number(event.target.value))
                        }
                    />
                    ...
                    <FormControl margin="normal" variant="outlined">
                        ...
                        <OutlinedInput
                            ...
                            type="number"
                            value={
                                stockSellPrice === undefined
                                    ? ""
                                    : stockSellPrice
                            }
                            inputProps={{ min: 0, step: "0.01" }}
                            onChange={(event) =>
                                setStockSellPrice(Number(event.target.value))
                            }
                        />
                    </FormControl>
