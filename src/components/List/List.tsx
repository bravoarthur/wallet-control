...
function List(item: IStock) {
    ...
    const lossOrProfit =
        currentPrice * Number(qtdStock) -
        Number(avaragePrice) * Number(qtdStock);
    ...
    return (
        <Tr
            role="trList"
            delClass={classProp}
            total={currentPrice === 0 ? 0 : lossOrProfit}
            key={stockName}
        >
            <TDgo role={`tickerField${stockName}`} onClick={_handleStockPage}>
                {stockName}
            </TDgo>
            <Td>{buyDate}</Td>
            <Td>{qtdStock}</Td>
            ...
            <Td>
                {currentPrice === 0 ? "0.00" : valuetionPercent.toFixed(2)}%
            </Td>
            <Td>{currentPrice === 0 ? "0.00" : lossOrProfit.toFixed(2)}</Td>
            <Td>
                {currentPrice === 0
                    ? "0.00"
                    : valuetionPercentYear(buyDate, currentPrice, avaragePrice)}
                %
            </Td>
            <Td>
                <DeleteForeverIcon
