...
function TradeList({ children }: Props) {
    ...
    const list = children
        ? tradeList.filter((item) => item.stockName === children)
        : tradeList;
        ...
        <Box display={"flex"} justifyContent={"center"}>
            ...
            >
                <ListItemButton onClick={(event) => handleClick(event)}>
                    <ListItemText>
                        <DivTradeList>
                            <Box fontWeight={700}>List of Sold Stocks</Box>
                        </DivTradeList>
                    </ListItemText>
                    <DivTradeList>
                                    ...
                                    <ListItemText>
                                        <DivTradeList role="trade-list-item">
                                            {`${item.sellDate} - Sold ${
                                                ...
                                                item.stockName
                                            } for $${item.sellPrice?.toFixed(
                                                2
                                            )} | ${item.valuetionPY.toFixed(
                                                2
                                            )}% | Balance: $ ${item.valuetion.toFixed(
                                                2
                                            )}`}
                                        </DivTradeList>
                                    </ListItemText>
