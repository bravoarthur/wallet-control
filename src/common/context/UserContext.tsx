...
export const UserProvider = ({children}: UserContextProps) => {
    ...
    const [user, setUser] = useState<IUser>(JSON.parse(`${localStorage.getItem(`currentUser`)}`))
    const [userList, setUserList] = useState<IUserList>(JSON.parse(`${localStorage.getItem(`userList`)}`) || [])
    const [newLogin, setNewLogin] = useState(true)
    
    function defineUserList (newItem:UserListItem) {

        const index = userList.findIndex(item => 
            item.value === newItem.label
        )
        if(index===-1) {
            ...
            setNewLogin(false)
            localStorage.setItem(`userList`, JSON.stringify([...userList, newItem]))
    ...
    return (
        ...
        <UserContext.Provider value={{user, userList, defineUserList, newLogin}}>
            {children}
